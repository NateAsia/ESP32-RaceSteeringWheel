...
class UIComponents:
    ...
    def setup_buttons(self):
        ...
        self.custom_button("REAR L (6)", "lightblue", row=0, flip=True, frame=self.bottom_frame)
        self.custom_button("REAR R (7)", "lightblue", row=0, col=2, frame=self.bottom_frame)

        connect_button = tk.Button(self.connected_frame, text="Connect", command=self.setup_serial_callback)
    ...
    def setup_labels(self):
        ...
        self.serial_monitor_label = tk.Label(self.master, text="PLEASE CONNECT TO STEERING WHEEL")
        self.serial_monitor_label.pack(pady=(0,15), side=tk.BOTTOM)

    def custom_button(self, name="BTN", color="green", flip=False, row=None, col=None, frame=None):
        if row is None:
            row = len(self.leds)
        if col is None:
            col = 0
